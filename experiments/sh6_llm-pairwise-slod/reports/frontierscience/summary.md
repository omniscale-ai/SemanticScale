# SH6 frontierscience — Results Summary

## Overall Accuracy

| Run slug | Accuracy | Correct | Answered | Errors |
|---|---|---|---|---|
| Qwen/Qwen3-30B-A3B-Instruct-2507_reasoning-none | 51.6% | 82 | 159 | 1 |
| R1-Distill-32B-cloudjudge_reasoning-auto | 18.2% | 27 | 148 | 12 |
| deepseek/deepseek-v3.2_reasoning-auto | 39.9% | 61 | 153 | 7 |
| deepseek/deepseek-v3.2_reasoning-auto_s1 | 37.8% | 56 | 148 | 12 |
| deepseek/deepseek-v3.2_reasoning-auto_s2 | 37.2% | 58 | 156 | 4 |
| deepseek/deepseek-v3.2_reasoning-auto_s3 | 37.7% | 58 | 154 | 6 |
| deepseek/deepseek-v3.2_reasoning-auto_s4 | 38.6% | 59 | 153 | 7 |
| deepseek/deepseek-v3.2_reasoning-auto_types-research | 0.0% | 0 | 57 | 3 |
| deepseek/deepseek-v3.2_reasoning-none_types-research | 0.0% | 0 | 60 | 0 |
| deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B_reasoning-auto | 5.0% | 1 | 20 | 0 |
| deepseek-ai/DeepSeek-R1-Distill-Qwen-32B_reasoning-auto | 20.9% | 31 | 148 | 12 |

## Qwen/Qwen3-30B-A3B-Instruct-2507_reasoning-none — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| biology | 53.3% | 16 | 30 | 0 |
| chemistry | 52.5% | 31 | 60 | 1 |
| physics | 50.0% | 35 | 70 | 0 |

## R1-Distill-32B-cloudjudge_reasoning-auto — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| physics | 24.2% | 15 | 70 | 8 |
| chemistry | 21.4% | 12 | 60 | 4 |
| biology | 0.0% | 0 | 30 | 0 |

## deepseek/deepseek-v3.2_reasoning-auto — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| chemistry | 47.5% | 28 | 60 | 1 |
| physics | 44.6% | 29 | 70 | 5 |
| biology | 13.8% | 4 | 30 | 1 |

## deepseek/deepseek-v3.2_reasoning-auto_s1 — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| physics | 44.4% | 28 | 70 | 7 |
| chemistry | 43.9% | 25 | 60 | 3 |
| biology | 10.7% | 3 | 30 | 2 |

## deepseek/deepseek-v3.2_reasoning-auto_s2 — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| chemistry | 43.1% | 25 | 60 | 2 |
| physics | 42.6% | 29 | 70 | 2 |
| biology | 13.3% | 4 | 30 | 0 |

## deepseek/deepseek-v3.2_reasoning-auto_s3 — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| chemistry | 46.6% | 27 | 60 | 2 |
| physics | 43.9% | 29 | 70 | 4 |
| biology | 6.7% | 2 | 30 | 0 |

## deepseek/deepseek-v3.2_reasoning-auto_s4 — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| physics | 45.5% | 30 | 70 | 4 |
| chemistry | 43.1% | 25 | 60 | 2 |
| biology | 13.8% | 4 | 30 | 1 |

## deepseek/deepseek-v3.2_reasoning-auto_types-research — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| biology | 0.0% | 0 | 20 | 1 |
| chemistry | 0.0% | 0 | 20 | 0 |
| physics | 0.0% | 0 | 20 | 2 |

## deepseek/deepseek-v3.2_reasoning-none_types-research — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| biology | 0.0% | 0 | 20 | 0 |
| chemistry | 0.0% | 0 | 20 | 0 |
| physics | 0.0% | 0 | 20 | 0 |

## deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B_reasoning-auto — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| physics | 5.0% | 1 | 20 | 0 |

## deepseek-ai/DeepSeek-R1-Distill-Qwen-32B_reasoning-auto — Per-Subject Accuracy

| Subject | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| physics | 25.8% | 16 | 70 | 8 |
| chemistry | 21.4% | 12 | 60 | 4 |
| biology | 10.0% | 3 | 30 | 0 |

## Qwen/Qwen3-30B-A3B-Instruct-2507_reasoning-none — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| research | 55.0% | 33 | 60 | 0 |
| olympiad | 49.5% | 49 | 100 | 1 |

## R1-Distill-32B-cloudjudge_reasoning-auto — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| olympiad | 30.3% | 27 | 100 | 11 |
| research | 0.0% | 0 | 60 | 1 |

## deepseek/deepseek-v3.2_reasoning-auto — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| olympiad | 64.2% | 61 | 100 | 5 |
| research | 0.0% | 0 | 60 | 2 |

## deepseek/deepseek-v3.2_reasoning-auto_s1 — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| olympiad | 58.3% | 56 | 100 | 4 |
| research | 0.0% | 0 | 60 | 8 |

## deepseek/deepseek-v3.2_reasoning-auto_s2 — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| olympiad | 60.4% | 58 | 100 | 4 |
| research | 0.0% | 0 | 60 | 0 |

## deepseek/deepseek-v3.2_reasoning-auto_s3 — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| olympiad | 58.6% | 58 | 100 | 1 |
| research | 0.0% | 0 | 60 | 5 |

## deepseek/deepseek-v3.2_reasoning-auto_s4 — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| olympiad | 59.8% | 58 | 100 | 3 |
| research | 1.8% | 1 | 60 | 4 |

## deepseek/deepseek-v3.2_reasoning-auto_types-research — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| research | 0.0% | 0 | 60 | 3 |

## deepseek/deepseek-v3.2_reasoning-none_types-research — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| research | 0.0% | 0 | 60 | 0 |

## deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B_reasoning-auto — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| olympiad | 5.0% | 1 | 20 | 0 |

## deepseek-ai/DeepSeek-R1-Distill-Qwen-32B_reasoning-auto — Per-Origin Accuracy

| Origin | Accuracy | Correct | Total | Errors |
|---|---|---|---|---|
| olympiad | 31.5% | 28 | 100 | 11 |
| research | 5.1% | 3 | 60 | 1 |
